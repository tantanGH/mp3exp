...
MP3再生時はデフォルトで周波数ハーフモードを使用するため V3.5 以上が必要です。オプション(`-q`)でフル周波数再生も可能です。その場合は V3.0/V3.1 でも大丈夫かもしれません。ただし 満開版 V4.0 での動作確認はしていません。(当方のエミュレータ環境だと V4.0 では pcm8pp.x が組み込めませんでした)
