...
XEiJでHDSファイルをマウントしたドライブ上で.S44/.PCMを逐次読み込みで再生する際、ディスク読み込みを行いDMAの複数チャンネルが同時に使われた瞬間にADPCMの音が乱れます。すべてオンメモリに取り込むMP3や、RAMDISK・ホストファイルシステム(HFS)上での再生であれば問題ありません。また、実機やXM6gでは問題ありません。XEiJはDMACのチャンネル間のプライオリティ設定によらず、HDのオートリクエスト最大速度が始まった場合はそれを最優先とする実装になっているためと思われます。
