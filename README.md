...
- [aubio](https://aubio.org/)などのツールを使って調べる

などの方法があります。自分は aubio を使っています。

    aubio tempo hogehoge.wav

などとしてWAVE形式に変換したデータを与えると予測BPM値を返してくれます。

また、いくつかテンプレートをBPMをずらして作ってみて再生し、しっくりくるものをベースに編集に入るのも良いと思います。出力されたテンプレートはダミーメッセージが入っていますがそのまますぐに SMR.X / MP3EXP.X で再生可能です。
