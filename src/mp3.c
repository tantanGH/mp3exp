...
printf("mp3_decode\n");
  ...
  do {
    ...
    if (result == -1) {
      ...
      break;  // no more data in this frame
    }
    ...
    MAD_PCM* pcm = &(decode->mad_synth.pcm);
#ifdef DEBUG
    printf("pcm length=%d,samplerate=%d,channels=%d\n",pcm->length,pcm->samplerate,pcm->channels);
#endif
    if (pcm->length > resample_buffer_len) {
      printf("error: insufficient MP3 decode buffer.\n");
      break;
    }
    ...
    if (pcm->channels == 2) {
      ...
      for (int32_t i = 0; i < pcm->length; i++) {
        ...
        int16_t x = ( pcm->samples[0][i] + pcm->samples[1][i] ) / 2 / 256 / resample_gain;
      ...
      for (int32_t i = 0; i < pcm->length; i++) {
        ...
        int16_t x = pcm->samples[0][i] / 256 / resample_gain;
        resample_buffer[ resample_ofs++ ] = x;
